import { type UUID128, type UUIDv47Key } from "..";
import {
  createKey,
  createKeyFromBuffer,
  decodeV4Facade,
...
class UUIDv47Examples {
  ...
  static keyManagementExample(): void {
    ...
    const knownKey = createKey(0x0123456789abcdefn, 0xfedcba9876543210n);
    ...
    const bufferKey = createKeyFromBuffer(keyBuffer);
    console.log(colorize.success("Key created from 16-byte buffer"));

    // Demonstrate key isolation
    const testUUID = parseUUID("01234567-89ab-7000-8000-123456789abc");
    const facade1 = encodeV4Facade(testUUID, randomKey);
    const facade2 = encodeV4Facade(testUUID, knownKey);
    const facade3 = encodeV4Facade(testUUID, bufferKey);

    console.log(colorize.info("Same UUID with different keys produces different facades:"));
    console.log(`  ${colorize.label("Random key:")}  ${colorize.uuid(formatUUID(facade1))}`);
    console.log(`  ${colorize.label("Known key:")}   ${colorize.uuid(formatUUID(facade2))}`);
    console.log(`  ${colorize.label("Buffer key:")}  ${colorize.uuid(formatUUID(facade3))}`);
    console.log("");
  }
  ...
  static errorHandlingExample(): void {
    ...
    try {
      ...
      encodeV4Facade(v4UUID, validKey);
    } catch (error) {
      console.log(
        colorize.success(
    ...
    try {
      ...
      const v7UUID = parseUUID("01234567-89ab-7000-8000-123456789abc");
      decodeV4Facade(v7UUID, validKey);
    } catch (error) {
      console.log(
        colorize.success(
    ...
    try {
      // Invalid key buffer
      createKeyFromBuffer(Buffer.from([1, 2, 3])); // Too short
    } catch (error) {
      console.log(
        colorize.success(
  ...
  static securityConsiderationsExample(): void {
    ...
    // Demonstrate key isolation
    const facade1 = encodeV4Facade(testUUID, key1);
    const facade2 = encodeV4Facade(testUUID, key2);

    console.log(colorize.success("Key isolation:"));
    console.log(colorize.info("  Same UUID, different keys produce different facades:"));
    console.log(`  ${colorize.label("Key1:")} ${colorize.uuid(formatUUID(facade1))}`);
    console.log(`  ${colorize.label("Key2:")} ${colorize.uuid(formatUUID(facade2))}`);
    console.log(
      `  ${colorize.label("Facades differ:")} ${!facade1.equals(facade2) ? colorize.success("true") : colorize.error("false")}`,
    );

    // Demonstrate tampering detection
    const originalFacade = encodeV4Facade(testUUID, key1);
    const tamperedFacade = Buffer.from(originalFacade);
    tamperedFacade[15] ^= 0x01; // Flip one bit

    const originalDecoded = decodeV4Facade(originalFacade, key1);
    const tamperedDecoded = decodeV4Facade(tamperedFacade, key1);
